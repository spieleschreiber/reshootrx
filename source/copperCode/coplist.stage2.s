...
red		SET 	97<<4		; start values
green	SET 	102<<4
blue	SET 	156<<4
redA	SET	red
greenA	SET	green
blueA	SET	blue
redFac		SET 	(red-87<<4)	; target values
greenFac	SET 	(green-92<<4)
blueFac		SET		(blue-116<<4)
...
rgbHighB	SET	((\2+\5)&$f0)<<4!(\3&$f0)!(\4&$f0)>>4
rgbLowB	SET	((\2+\5)&$f)<<8!(\3&$f)<<4!(\4&$f)
rgbHighC	SET	((\2+\5*2)&$f0)<<4!(\3&$f0)!(\4&$f0)>>4
rgbLowC	SET	((\2+\5*2)&$f)<<8!(\3&$f)<<4!(\4&$f)
	...
	dc.w COLOR31
	dc.w rgbHighC
	CMOVE BPLCON3,BANK2F|BANK1F|BANK0F|BRDRBLNKF|LOCTF
	dc.w COLOR29
	dc.w rgbLow
	dc.w COLOR30
	dc.w rgbLowB
...
statusLow	SET (rgbLow0old-rgbLow0)
	...
	IFNE statusHigh|statusLow
		...
		CMOVE BPLCON3,BRDRBLNKF|PF2OF1F|PF2OF0F
			...
			dc.w COLOR00
			dc.w rgbHigh0
			IF (parSpriteY>128)!(parSpriteY<140)
			ENDIF
			ENDIF
		ENDIF
...
upgrColor	SET 	$6af
	...
	;CMOVE BPL2MOD,0
		...
		CMOVE color14,$210		; reset ship color + sprite parallax color
		CMOVE COLOR15,$eef	; reset ship color
			CMOVE SPR6POS,$3058	; reposition scroll sprites
		CMOVE SPR6CTL,$2902
		CMOVE SPR7POS,$3000
		CMOVE SPR7CTL,$3102
				CMOVE BPLCON3,$f000!BRDRBLNKF
		CMOVE COLOR29,$669;$667		;97 102 116
		CMOVE COLOR30,$667	;566
		CMOVE COLOR31,$668;455
		
		ENDIF
		
		;COPCOLSPLIT $32,$f0,$f,0,$1
		IF 1=0
	...
	ENDIF
		...
		IF (parSpriteY>spriteScoreYPosition+4)
		; wait needed for player sprite split 
...
;	CMOVE NOOP,0
	...
	IF (parSpriteY=displayWindowStart+125)
		...
		CMOVE BPL2MOD,-40*2
	ENDIF
	IF (parSpriteY=displayWindowStart+248)
		CMOVE BPL2MOD,0	; fix for bottom vfx display bug
	ENDIF
...
;	CMOVE SPR7CTL,0
		...
		CMOVE COLOR01,$4ff	; ugly color fix for CD32 color bug
		CMOVE BPLCON3,LOCTF|BRDRBLNKF	; prep colors for next frame update
