...
.add	SET			(mainPlaneWidth*4*(escalateStart+escalateHeight-displayWindowStart-0))
	...
	;TOSHELL		 d7, "=viewPositionPointer"
	clr.w		d0								; scroll offset modifier
	
	;move.l		viewPositionAdd(a3),d4
	tst.l		viewPositionAdd(a3)
	bmi			modifyTilePointer
tilePointerModified
	lsr			#7,d7
	add.w		 d2,d7
...
.swapGfxBuffers
	clr.l		d1
	move		viewPositionPointer(a3),d3
	move.w		d3,d1
	lsr			#5,d1
	...
	add.l		d1,d7
				...
				movem.l		d5-d7,(a6)																	; write modified mainPlanePointer for use in blittermanager / write to bpltpt at copperlist 
			
	move.l		d5,a1
	move.l		d6,a2
	move.l		d7,a4																		; a1/a2/a4 = mainPlanePointers for tile drawing
	lsr			 #1,d3
	;add.w		d0,d3
	andi.w		#$1e<<1,d3
	;move.w		#$0<<1,d3
...
smoothScrollRet
	...
	tst			plyCollided(a5)						; ply death seq running? Do not modify y-scrolling
	bne			.noVertHorzScrolling
...
.tileMemSize	=			(tileHeight-1)*tileWidth/2
	...
	ror.w		#3,d5					; (mainPlaneWidth*mainPlaneDepth)*256 = $a000
				move.l		#mainPlaneWidth*mainPlaneDepth*256,d5															; offset between single bitmap layers
	move.l		#$0f0f0f0f,d6
...
.getTile
	...
	move		tileMapWidth(pc),d2
	lea			(a0,d2.w),a0																; 
			; switch to next row within Tilemap
...
.drawTilesMirrored
	...
	swap		d7
.offset			SET			((mainPlaneWidth-tileWidth/8)*tileHeight*mainPlaneDepth)-16-mainPlaneWidth		; offset to next tile bitmap  on screen
;.offset			SET			((mainPlaneWidth-tileWidth/8)*tileHeight*mainPlaneDepth)-16+mainPlaneWidth*mainPlaneDepth	; offset to next 
	lea			.offset(a1),a1
	lea			.offset(a2),a2
	lea			.offset(a4),a4
...
modifyTilePointer
	move.w		#-1,d0
	add.w		#256,d7
	move.w		#mainPlaneWidth*mainPlaneDepth-(mainPlaneWidth-4),d4																	
